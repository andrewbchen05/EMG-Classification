...
class AddGaussianNoise:
    std: float = 0.01  # Default standard deviation for noise
...
@dataclass
class RandomCrop:
    """Randomly crops a tensor along the time dimension.

    Args:
        crop_size (int): The size of the crop (i.e., the length along the time dimension).
        time_dim (int): The dimension corresponding to the time axis (default is 0).
    """
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        # If the tensor size along the time dimension is smaller than the crop_size, return the tensor as is
        if tensor.shape[self.time_dim] < self.crop_size:
            return tensor

        # Randomly choose a start index for the crop
        start_idx = np.random.randint(0, tensor.shape[self.time_dim] - self.crop_size + 1)
        slices = [slice(None)] * tensor.ndimension()
        slices[self.time_dim] = slice(start_idx, start_idx + self.crop_size)
...
class GaussianNoise:
    ...
    Args:
        ...
        p (float): Probability of applying the noise (default: 1.0, i.e., always applied).
    """
...
class FrequencySmoothing:
    """Applies a low-pass filter to the spectrogram data.

    Args:
        cutoff_freq (int): The cutoff frequency for the low-pass filter.
    """

    cutoff_freq: int = 30

    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        tensor[..., :self.cutoff_freq] = tensor[..., :self.cutoff_freq].mean(dim=-1, keepdim=True)
        return tensor
